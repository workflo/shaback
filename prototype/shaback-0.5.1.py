...
SPLIT_BLOCK_SIZE = 1024 * 1024 
ALL_COMMANDS = ['backup', 'gc', 'init', 'restore', 'list', 'help']
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ha:zvx:X:Z:s:S:d:n:tofy",
				   ["help", "verbose", "debug",
                                    "archive=", "zip", 
				    "exclude=", "exclude-from=", "dont-zip-from=",
				    "one-file-system",
				    "split=", "split-from=",
				    "destination=", "name=",
				    "totals", "force", "yes"])
	...
	elif o in ("-Z", "--dont-zip-from"):
	    dontZip += loadGlobPatterns(a);
	elif o in ("-z", "--zip"):
	    doZip = True
	elif o in ("-s", "--split"):
	    splitFiles += [a]
...
class Archive:
	...
	global verbose
        ...
        [fh, path] = tempfile.mkstemp(suffix='.shaback')
	self.tmpIndexFilename = path

	if self.doZip:
	    self.indexFile += ".gz"
	    self.tmpIndexFile = gzip.open(path, 'wt')
	else:
	    self.tmpIndexFile = open(path, 'wt')

	self.indexFileWriter = csv.writer(self.tmpIndexFile, delimiter=';', quotechar='"')
	if verbose:
    ...
    def dontZipFile(self, filename):
	for pattern in self.dontZip:
	    if fnmatch.fnmatch(filename, pattern):
		return True
	return False
    ...
    def setZip(self, doZip, dontZip):
	self.doZip = doZip
	self.dontZip = dontZip
	...
	if entry.isDir:
	    ...
	    elif os.path.isfile(sourceFile + ".gz"):
		isBlockList = False
		fh = gzip.open(sourceFile + ".gz")
	    elif os.path.isfile(sourceFile + ".blist"):
		isBlockList = True
		fh = open(sourceFile + ".blist", "rt")
	    elif os.path.isfile(sourceFile + ".blist.gz"):
		isBlockList = True
		fh = gzip.open(sourceFile + ".blist.gz", "rt")
	    else:
	...
	for child in os.listdir(self.indexDir):
	    if fnmatch.fnmatch(child, "*-????????_??????.shaback") or fnmatch.fnmatch(child, "*-????????_??????.shaback.gz"):
...
class BackupFile:
	...
	elif os.path.isfile(self.archiveFilename + ".gz"):
	    self.alreadyBackedUp = True
	    self.isZipped = True
	elif os.path.isfile(self.archiveFilename + ".blist"):
	    self.alreadyBackedUp = True
	    self.isSplit = True
	elif os.path.isfile(self.archiveFilename + ".blist.gz"):
	    self.alreadyBackedUp = True
	    self.isZipped = True
	    self.isSplit = True
	else:
    ...
    # Decides whether to split or gzip the file.
    #
    def copyFileToArchive(self):
	global verbose, MIN_ZIP_SIZE, BLOCK_SIZE, SPLIT_BLOCK_SIZE

	self.isZipped = self.archive.doZip and not self.archive.dontZipFile(self.sourceFile) and self.stat[ST_SIZE] > MIN_ZIP_SIZE
	...
	if self.stat[ST_SIZE] > SPLIT_BLOCK_SIZE * 5 and self.archive.splitFile(self.sourceFile):
		...
		self.archiveFilename += ".gz"
		output = gzip.open(self.archiveFilename + ".tmp", "wb")
		...
		if self.content != None:
		    output = open(self.archiveFilename + ".tmp", "wb")
		    try:
			output.write(self.content)
		    finally:
			output.close()
		else:
		    shutil.copyfile(self.sourceFile, self.archiveFilename + ".tmp")
	...
	if self.isZipped:
	    ...
	    blockList = open(self.archiveFilename + ".tmp", "wt")
	...
	try:
		...
		if os.path.exists(blockFilename) or os.path.exists(blockFilename + ".gz"):
		    if verbose:
			print "   . " + blockFilename
		else:
		    if verbose:
			print "   B " + blockFilename
		    dirname = os.path.dirname(blockFilename)
		    if not os.path.isdir(dirname):
			os.makedirs(dirname)

		    if self.isZipped:
			blockFilename = blockFilename + ".gz"
			output = gzip.open(blockFilename + ".tmp", "wb")
		    else:
			output = open(blockFilename + ".tmp", "wb")
		    try:
			output.write(block)
...
class IndexFile:
	...
	self.path = os.path.join(archive.indexDir, filename)

	if fnmatch.fnmatch(self.filename, "*.shaback.gz"):
	    self.isZipped = True
	elif fnmatch.fnmatch(self.filename, "*.shaback"):
	    self.isZipped = False
	else:
...
class SplitFileReader:
	...
	self.sha1 = sha1
	self.filename = archive.makeArchiveFilename(sha1)
	
	if os.path.isfile(self.filename + ".blist"):
	    self.isZipped = False
	    self.filename += '.blist'
	elif os.path.isfile(self.filename + ".blist.gz"):
	    self.isZipped = True
	    self.filename += '.blist.gz'
	else:
	...
	if self.errors > 0 and not force:
		...
		for index1 in range(0, 256):
			...
			if remainder not in self.tree[index0][index1]:
			    ...
			    if verbose:
				print " deleting " + level0Name + level1Name + remainder
			    if yes:
				os.remove(path)
...
def usage(command = None):
	...
	print "usage: shaback -a|--archive              Location of existing archive directory."
	print "               [-z|--zip]                gzip compress files."
	...
	print "               [-Z|--dont-zip-from=FILE] Read filepattern of files not to be compressed from a file."
